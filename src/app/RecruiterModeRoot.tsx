...
import { FlickeringGrid } from "@/app/components/magic/FlickeringGrid";
...
export function RecruiterModeRoot() {
  ...
  const sectionLinks = useMemo(
    ...
    [labels.contact, labels.experience, labels.projects, labels.skills, labels.tools],
  );
  ...
  const featuredProjects = useMemo(() => {
    const selected = projects.filter((project) => featuredProjectIds.has(project.id));
    return selected.length > 0 ? selected : projects.slice(0, 3);
  }, [projects, featuredProjectIds]);
  const archivedProjects = useMemo(
    () => projects.filter((project) => !featuredProjects.some((item) => item.id === project.id)),
    [projects, featuredProjects],
  );
  const recruiterAboutCards = useMemo(
    () => [
      {
        title: "How I Work",
        text: "I bridge product thinking, UX decisions, and implementation so ideas keep moving from concept to shipped experience.",
      },
      {
        title: "Best Fit",
        text: "Product-minded engineering roles where execution quality, user experience, and iteration speed all matter.",
      },
      {
        title: "Signal",
        text: `${experience.length} core roles and ${projects.length} builds across support systems, interactive products, and workflow tools.`,
      },
    ],
  ...
  return (
    <div className="recruiter-shell relative min-h-screen w-full overflow-hidden bg-[#F3F6F8] px-4 py-10 text-[#1E2A33] transition-colors dark:bg-[#0E151B] dark:text-[#ECF3F7] sm:px-6">
      <FlickeringGrid className="opacity-60 dark:opacity-45" />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_12%_8%,rgba(89,169,106,0.08),transparent_30%),radial-gradient(circle_at_88%_14%,rgba(74,144,226,0.08),transparent_32%),radial-gradient(circle_at_62%_78%,rgba(241,154,62,0.08),transparent_36%)] dark:bg-[radial-gradient(circle_at_14%_10%,rgba(89,169,106,0.06),transparent_30%),radial-gradient(circle_at_84%_14%,rgba(74,144,226,0.08),transparent_30%),radial-gradient(circle_at_62%_78%,rgba(241,154,62,0.06),transparent_36%)]" />
      <main className="relative z-10 mx-auto grid max-w-7xl gap-6 lg:grid-cols-[19rem_minmax(0,1fr)]">
                  ...
                  <button
                    ...
                    className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-all duration-200 ${
                      activeSectionId === section.id
                        ? "border-[#59A96A]/45 bg-[#E8F6EC] text-[#23653A] shadow-[0_0_0_1px_rgba(89,169,106,0.08)] dark:border-[#59A96A]/35 dark:bg-[#59A96A]/16 dark:text-[#CDEFD5]"
                        : "border-[#C8D4DE] bg-[#EEF4F8] text-[#1F3242] hover:bg-[#E1EAF2] dark:border-white/15 dark:bg-white/6 dark:text-[#F1F6FA] dark:hover:bg-white/15"
                    }`}
            ...
            <div className={`${panelBase} p-6`}>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                {recruiterAboutCards.map((item) => (
                  <article
                    key={item.title}
                    className="rounded-2xl border border-[#D5E0E8] bg-[#F7FAFC] p-4 transition-colors duration-300 dark:border-white/12 dark:bg-white/5"
                  >
                    <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#5C6A77] dark:text-[#B8C4CD]">
                      {item.title}
                    </p>
                    <p className="mt-2 text-sm leading-relaxed text-[#2A3B49] dark:text-[#ECF3F7]">
                      {item.text}
                    </p>
                  </article>
                ))}
            ...
            <div className={`${panelBase} p-6`}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="rounded-full border border-[#C8D4DE] bg-[#EEF4F8] px-3 py-1.5 text-xs font-semibold text-[#1F3242] transition-colors hover:bg-[#E1EAF2] dark:border-white/15 dark:bg-white/8 dark:text-[#F1F6FA] dark:hover:bg-white/16"
                  aria-expanded={showAllProjects}
                >
                  {showAllProjects
                    ? `Show Featured Only (${featuredProjects.length})`
                    : `Show All Projects (${projects.length})`}
                </button>
              </div>

              {!showAllProjects && (
                <div className="mt-4 rounded-2xl border border-[#D5E0E8] bg-[#F7FAFC] px-4 py-3 text-sm text-[#334656] dark:border-white/10 dark:bg-white/5 dark:text-[#D6E0E7]">
                  <span className="font-semibold text-[#102133] dark:text-[#F7FBFF]">
                    Quick recruiter scan:
                  </span>{" "}
                  {featuredProjects.length} featured builds surfaced first. The remaining {archivedProjects.length} projects stay one click away.
                </div>
              )}
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-2">
                {(showAllProjects
                  ...
                  const isFeatured = featuredProjects.some((item) => item.id === project.id);
                  return (
                  <article
                    key={project.id}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {isFeatured && (
                          <span className="inline-flex items-center rounded-full border border-[#59A96A]/35 bg-[#E8F6EC] px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#2D6D3B] dark:border-[#59A96A]/30 dark:bg-[#59A96A]/14 dark:text-[#BDE8C5]">
                            Featured
                          </span>
                        )}
                      </div>
                    ...
                    <div className="mt-3 rounded-xl border border-[#59A96A]/15 bg-[#EEF7F1] px-3 py-2 dark:border-[#59A96A]/18 dark:bg-[#0E1C18]/55">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#2D6D3B] dark:text-[#BDE8C5]">
                        Why it mattered
                      </p>
                      <p className="mt-1 text-xs leading-relaxed text-[#2A3B49] dark:text-[#D6E0E7]">
                        {project.impactBullets[0]}
                      </p>
                    </div>
                    <p className="mt-3 text-xs text-[#5C6A77] dark:text-[#B8C4CD]">{project.status.access}</p>
                    ...
                    <div className="mt-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {project.stack.slice(0, 7).map((tool) => (
                          <span
                            key={`${project.id}-${tool}`}
                        ...
                        {project.stack.length > 7 && (
                          <span className="inline-flex items-center rounded-full border border-[#C8D4DE] bg-[#EEF4F8] px-2.5 py-1 text-[11px] text-[#5C6A77] dark:border-white/12 dark:bg-white/8 dark:text-[#B8C4CD]">
